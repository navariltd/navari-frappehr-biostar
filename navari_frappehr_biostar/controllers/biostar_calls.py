    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                frappe.throw("Login failed or cookie not found")
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not records:
                        # Filter out entries without check-in/out
                        self.attendance_logs = [
                            log
                    ...
                    self.attendance_logs.extend(records)
                    offset += limit
                else:
                    frappe.throw(
...
def check_relieving_date(employee):
    if employee.relieving_date:
        ...
        first_day_of_month = frappe.utils.formatdate(
            frappe.utils.get_first_day(frappe.utils.today()), "dd-MM-YYYY"
        )
...
def add_checkin_logs(start_date=None, end_date=None):
    employees = frappe.db.get_all(
        ...
        filters={
            ...
            "relieving_date": ["<=", frappe.utils.get_first_day(frappe.utils.today())],
        },
...
@frappe.whitelist()
def add_checkin_logs_for_specified_date(start_date, end_date, status):
    if not status:
        return
...
def fetch_single_employee_attendance(start_date, end_date, employee):
    ...
    if not employee_doc.attendance_device_id:
        frappe.throw(
            f"Employee {employee_doc.name} does not have an attendance device ID."
        )

    biostar = BiostarConnect(username=username, password=password)
    biostar.get_attendance_report(
        employee_doc.attendance_device_id, start_date, end_date
    )
    biostar.format_attendance_logs()
    biostar.create_punch_logs()
